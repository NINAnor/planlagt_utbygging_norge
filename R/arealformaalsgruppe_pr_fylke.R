# --------------------------------------------------------------------------- #
# Name: arealformaalsgruppe_pr_fylke.R
# Date: 2023-08-23
...
ggplot(summary_data, aes(x = reorder(fylke, -total_area_km2), y = total_area_km2)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(x = "Fylke", y = "Total Area (sq. km)", title = "Total Planned Area by Fylke") +
  theme_minimal() +
...
fylke_order <- c("Trøndelag", "Innlandet", "Viken", "Troms og finnmark", "Vestland", "Agder", "Nordland", "Vestfold og telemark", "Møre og romsdal", "Rogaland")
...
my_plot <- ggplot(summary_data, aes(x = fylke, y = total_area_km2, fill = arealformalsgruppe)) +
  ...
  scale_fill_manual(values = arealformalsgruppe_colors, name = "Arealformålsgruppe") +  # Set the colors for each "arealformalsgruppe" and name the legend
...
ggsave("plots_and_tables/samlet_areal_fylke_arealformaalsgruppe.jpg", plot = my_plot, device = "jpeg", dpi = 300, width = width_inches, height = height_inches)
...
write_excel_csv(final_summary, "plots_and_tables/sum_arealformaalsgrupper_fylkesvis.csv")
